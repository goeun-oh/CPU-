...
void func4(uint32_t *prevTime, uint32_t *data , uint32_t *us_dist);
...
void uart_annc(uint32_t data); 
void uart_us_mode1();
void uart_us_mode2();
void uart_us_mode3();
void uart_temp_mode4();
void uart_temp_mode5();
...
enum {IDLE, IDLE_T, US_READY, TEMP_READY, FUNC1, FUNC2, FUNC3, FUNC4 , FUNC5, FUNC6};
...
int main() {
    ...
    while (1) {
        ...
        // 현재 상태에 맞는 기능 실행
        switch (state) {
            case FUNC1: func1(&func1PrevTime, &func1Data, &us_dist); break;
            case FUNC2: func2(&func2PrevTime, &func2Data, &us_dist); break;
            case FUNC3: func3(&func3PrevTime, &func3Data, &us_dist); break;
        }

        // UART입력을 통해 상태 전환
        uint32_t btn = Button_getState(GPIOD);
        if (!(UART_state(UART) & (1<<0))) { // rx empty가 아닐때 읽기
                uart_rdata = UART_readData(UART);
                uart_annc(uart_rdata); 
                // UART_writeData (UART, uart_rdata);
                // UART_writeData (UART, '\n');
        }
        switch (state) {
            case IDLE:
                if (uart_rdata == 'u' || uart_rdata == 'U') state = US_READY;
                else state = IDLE;
                break;
            case US_READY:
                if (uart_rdata == '1') state = FUNC1;
                else if (uart_rdata == '2') state = FUNC2;
                else if (uart_rdata == '3') state = FUNC3;
                else if (uart_rdata == 'x' || uart_rdata == 'X') state = IDLE;
                else state = US_READY;
                break;

            case FUNC1:
                if (uart_rdata == '2') state = FUNC2;
                else if (uart_rdata == '3') state = FUNC3;
                else if (uart_rdata == 'x' || uart_rdata == 'X') state = IDLE;
                else state = FUNC1;
                break;
...
// void uart_annoucement(uint32_t data){
//     if
// }
