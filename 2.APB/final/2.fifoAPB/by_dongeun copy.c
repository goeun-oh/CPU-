...
uint32_t FIFO_writeCheck(FIFO_TypeDef *fifo);
void FIFO_writeData(FIFO_TypeDef *fifo, uint32_t data);
...
{   
    ...
    if ((FIFO_writeCheck(FIFO) & (one<<1)) == 0){
    FIFO_writeData(FIFO, 0x22);
    FIFO_writeData(FIFO, 0x02);
    FIFO_writeData(FIFO, 0x03);
    FIFO_writeData(FIFO, 0x04);
    FIFO_writeData(FIFO, 0x05);
    } 

    if ((FIFO_writeCheck(FIFO)& (one)) == 0){
    FIFO_readData(FIFO);
    FIFO_readData(FIFO);
    FIFO_readData(FIFO);
    FIFO_readData(FIFO);
